...
{ 
    ...
    while (k < tab1.size() + tab2.size()) // REMPLIR LE DERNIER TABLEAU avec TOUTES LES VALEURS DES DEUX ARBRES
    {
        i = 0; j = 0;

        while (limite1 < tab1.size() && i < pow(2,co))
        {
        ...
        while (i < pow(2,co) && limite1 >= tab1.size())
        {
            elem_tmp.etiq = -1;
        ...
        while (limite2 < tab2.size() && j < pow(2,co))
        {
        ...
        while (j < pow(2,co) && limite2 >= tab2.size())
        {
            elem_tmp.etiq = -1;
            elem_tmp.etiq2 = (char) 0;
            tab3.push_back(elem_tmp);
