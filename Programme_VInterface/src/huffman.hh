...
public:
    ...
    bool lecture_traitements_C(std::string txt); // recupere le texte à traiter, initialise le vecteur arbres en stockant le code ascii de chaque caractére du texte
                                // convertir le nombre d'ocurrence de chaque caractére en un pourcentage d'occurence par rapport à la taille du texte
    std::string lecture_traitements_DE(std::string txt); // fonction qui recupere le code à decrypter vérifie sa validité puis essaie de le décrypter et retourne le résultat dans le cas favorable sinon une erreur 
    ...
    std::string genere_code(); // génere le code correspondant au texte à partir de l'arbre trouvé dans la fonction fusionner_arbres 
